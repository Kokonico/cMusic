...
def main():
    ...
    match os_name:
        case "nt":
            ...
            try:
                ...
                print("Windows is not supported, and will most likely not work correctly, this program will now exit.")
                exit(1)
        case "posix":
    ...
    if tmux_check.returncode != 0:
        ...
        specific_os = subprocess.run(["uname", "-s"], stdout=subprocess.PIPE, stderr=subprocess.PIPE).stdout.decode("utf-8").strip()
        if specific_os == "Darwin":
            MAIN.log(Info("macOS (superior OS) detected, requesting installation of tmux via Homebrew."))
            ...
            if tmux_install.lower() == "y":
                ...
                if brew_install.returncode != 0:
                    ...
                    print("Failed to install tmux via Homebrew, do you have Homebrew installed?")
                    exit(1)
            else:
                MAIN.log(Warn("tmux is required for background playback, please install it."))
                print("tmux is required for background playback, please install it in some way or another.")
                exit(1)
            ...
            MAIN.log(Warn("tmux is not installed."))
            print("tmux is not installed, please install it via your package manager or add it to your PATH.")
            exit(1)
